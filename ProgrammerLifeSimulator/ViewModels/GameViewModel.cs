...
public class GameViewModel : ViewModelBase
{
    ...
    public GameViewModel(Player player, IGameEngineService gameEngine, IRandomService randomService)
    {
        ...
        Month = 1;
        SelectOptionCommand = new AsyncRelayCommand<EventOption>(SelectOptionAsync);
    ...
    private async Task SelectOptionAsync(EventOption? option)
    {
        if (option is null || IsGameCompleted || IsProcessing) return;
        
        try 
        {
            IsProcessing = true; // 开启状态锁
        ...
        {
            IsProcessing = false; // 无论是否出错，最后都要解锁
        }
    }
